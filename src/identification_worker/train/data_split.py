...
def split_ratio(data: pd.DataFrame, train_split: float, test_split: float):
    ...
    train_counts = {
        k: int(np.ceil(v * train_split)) for k, v in individuals_counts.items()
    }
    test_counts = {
        k: int(np.ceil(v * test_split)) for k, v in individuals_counts.items()
    }
    ...
    test_data = test_data[test_data.identity.isin(train_names)].reset_index(
        drop=True
    )
