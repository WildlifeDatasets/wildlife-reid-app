...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        """Run inference healthcheck for a specific user and their UploadedArchives."""
        ...
        self.stdout.write("\n--- [PART 1] Taxon Classification Healthcheck ---")
        ua_taxon = (
            UploadedArchive.objects.filter(owner=caiduser, contains_single_taxon=False).order_by("-uploaded_at").first()
        )
        if not ua_taxon:
            self.stdout.write(
                self.style.WARNING(
                    "Warning: No archive for Taxon Classification found (contains_single_taxon=False). Skipping Part 1."
                )
            )
        else:
        ...
        self.stdout.write("\n--- [PART 2] Identification Healthcheck ---")
        ua_id = (
            UploadedArchive.objects.filter(owner=caiduser, contains_single_taxon=True, contains_identities=False)
            .order_by("-uploaded_at")
            .first()
        )

        if not ua_id:
            raise InferenceHealthcheckError(
                "Error: No UploadedArchive found for identification healthcheck "
                "(contains_single_taxon=True, contains_identities=False)."
            )

        if not caiduser.workgroup or not caiduser.workgroup.identification_model:
            raise InferenceHealthcheckError(
                f"Error: Workgroup '{caiduser.workgroup}' has no identification_model assigned. "
                + "Cannot run identification."
            )
    ...
    def run_taxon_check(self, ua):
        """Trigger the taxon classification task and poll for completion."""
    ...
    def run_identification_check(self, ua, workgroup):
        """Trigger the identification task and poll for completion."""
        ...
        if not success:
            raise InferenceHealthcheckError(
                "Failed to trigger identification (run_identification returned False - possibly no observations found)."
            )
    ...
    def poll_status(self, ua, field_name, success_statuses, timeout_seconds=900):
        """Poll the specified status field of the UploadedArchive until success or fail/times out."""
        poll_interval = 10
